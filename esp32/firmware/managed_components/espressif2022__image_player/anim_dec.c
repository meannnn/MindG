...
{
    // Free any existing allocations before parsing new header
    ...
    if (strncmp(header->format, "_S", 2) == 0) {
        ...
        header->split_lengths = (uint16_t *)heap_caps_malloc(header->splits * sizeof(uint16_t), MALLOC_CAP_DEFAULT);
        ...
        header->palette = (uint8_t *)heap_caps_malloc(header->num_colors * 4, MALLOC_CAP_DEFAULT);
        if (header->palette == NULL) {
            ESP_LOGE(TAG, "Failed to allocate memory for palette");
            heap_caps_free(header->split_lengths);
        ...
        header->palette = (uint8_t *)heap_caps_malloc(file_length + 1, MALLOC_CAP_DEFAULT);
        if (header->palette == NULL) {
...
{
    Node* node = (Node*)heap_caps_calloc(1, sizeof(Node), MALLOC_CAP_DEFAULT);
    return node;
}
...
static void free_tree(Node* node)
{
    ...
    // Use iterative approach with fixed-size stack to avoid stack overflow
    // Maximum depth is limited by number of symbols (typically 256 for 8-bit)
    // Use a fixed-size array stack (sufficient for most Huffman trees)
    ...
    while (stack_top > 0) {
        ...
        // Push children onto stack before freeing current node
        if (current->right && stack_top < MAX_TREE_DEPTH) {
            stack[stack_top++] = current->right;
        } else if (current->right) {
            // If stack is full, fall back to recursive for remaining subtree
            free_tree(current->right);
        }
        
        if (current->left && stack_top < MAX_TREE_DEPTH) {
