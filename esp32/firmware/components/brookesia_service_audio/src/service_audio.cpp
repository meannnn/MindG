...
bool Audio::on_start()
{
    ...
    BROOKESIA_CHECK_ESP_ERR_RETURN(
        av_processor_audio_manager_init(&manager_config), false, "Failed to initialize audio manager"
    );
    ...
    BROOKESIA_CHECK_ESP_ERR_RETURN(av_processor_audio_playback_open(&playback_config), false, "Failed to open playback");
...
void Audio::on_stop()
{
    ...
    BROOKESIA_CHECK_ESP_ERR_EXECUTE(av_processor_audio_playback_close(), {}, {
    ...
    BROOKESIA_CHECK_ESP_ERR_EXECUTE(av_processor_audio_manager_deinit(), {}, {
...
bool Audio::play_url_internal(const std::string &url, const AudioPlayUrlConfig &config)
{
    ...
    auto do_play = [this, url, play_count, need_loop, new_session_id]() {
        ...
        if (av_processor_audio_playback_play(url.c_str()) != ESP_OK) {
...
std::expected<void, std::string> Audio::function_play_control(const std::string &action)
{
    ...
    case AudioPlayControlAction::Pause:
        if (av_processor_audio_playback_pause() != ESP_OK) {
    ...
    case AudioPlayControlAction::Resume:
        if (av_processor_audio_playback_resume() != ESP_OK) {
    ...
    case AudioPlayControlAction::Stop:
        ...
        if (av_processor_audio_playback_stop() != ESP_OK) {
...
bool Audio::start_encoder(const AudioEncoderDynamicConfig &config)
{
    ...
    BROOKESIA_CHECK_ESP_ERR_RETURN(av_processor_audio_recorder_open(&recorder_config), false, "Failed to open recorder");
#else
    {
    ...
    lib_utils::FunctionGuard close_recorder_guard([this]() {
        BROOKESIA_LOG_TRACE_GUARD_WITH_THIS();
        BROOKESIA_CHECK_ESP_ERR_EXECUTE(av_processor_audio_recorder_close(), {}, {
    ...
    [this, fetch_data_size = config.fetch_data_size, fetch_interval_ms = config.fetch_interval_ms]() {
        ...
        try {
            while (!boost::this_thread::interruption_requested()) {
                ret_size = av_processor_audio_recorder_read_data(data.get(), fetch_data_size);
