...
#include "systems/speaker/esp_brookesia_speaker_manager.hpp"
#include "systems/speaker/widgets/gesture/esp_brookesia_gesture.hpp"
...
using Manager = esp_brookesia::systems::speaker::Manager;
using QuickSettings = esp_brookesia::systems::speaker::QuickSettings;
using GestureInfo = esp_brookesia::systems::speaker::GestureInfo;
}
...
bool SettingsManager::processInit()
{
    ...
    // Only set QuickSettings if running in Speaker system (QuickSettings is Speaker-specific)
    auto speaker_system = app.getSystem();
    if (speaker_system != nullptr) {
        // Only set wifi icon state if QuickSettings has been initialized
        if (speaker_system->getDisplay().getQuickSettings().isBegun()) {
            ESP_UTILS_CHECK_FALSE_RETURN(
                speaker_system->getDisplay().getQuickSettings().setWifiIconState(
...
bool SettingsManager::processRunUI_ScreenAbout()
{
...

    {
        ...
        ESP_UTILS_CHECK_FALSE_RETURN(
            cell_firmware->updateRightMainLabel(IDF_VER), false,
...
bool SettingsManager::processOnWlanUI_Thread()
{
    ...
    auto system = app.getSystem();
    QuickSettings *quick_settings = nullptr;
    if (system != nullptr) {
        quick_settings = &system->getDisplay().getQuickSettings();
    }
    auto &storage_service = StorageNVS::requestInstance();
    ...
    if (is_wifi_event) {
        ...
        case WIFI_EVENT_STA_START:
            // Show status bar WLAN icon (only if QuickSettings available)
...
bool SettingsManager::processOnUI_ScreenWlanControlSwitchChangeEvent(lv_event_t *e)
{
    ...
    // Show/Hide status bar WLAN icon (only if running in Speaker system)
    auto speaker_system = app.getSystem();
    if (speaker_system != nullptr) {
        // Only set wifi icon state if QuickSettings has been initialized
        if (speaker_system->getDisplay().getQuickSettings().isBegun()) {
