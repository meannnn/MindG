...
#include "stylesheets/360_360/stylesheet.hpp"
...
bool Settings::addStylesheet(systems::speaker::Speaker *speaker, const SettingsStylesheetData *data)
{
...
bool Settings::run()
{
    ...
    systems::speaker::Speaker *speaker = getSystem();
    ESP_UTILS_CHECK_NULL_RETURN(speaker, false, "Invalid speaker");
...
bool Settings::init()
{
    ...
    if (getStylesheetCount() == 0) {
        static const auto default_stylesheet = SETTINGS_UI_360_360_STYLESHEET_DARK();
        ESP_UTILS_LOGW("No stylesheet is added, adding default stylesheet");
        ESP_UTILS_CHECK_FALSE_RETURN(
            addStylesheet(&default_stylesheet), false, "Failed to add default stylesheet"
        );
    }

    systems::base::Context *context = getSystemContext();
    ESP_UTILS_CHECK_NULL_RETURN(context, false, "Invalid context");
    gui::StyleSize display_size = {};
    ESP_UTILS_CHECK_FALSE_RETURN(context->getDisplaySize(display_size), false, "Failed to get display size");
    ...
    if (_active_stylesheet.name == nullptr) {
        ...
        // If no exact match found, try to use the first available stylesheet
        if (default_find_data == nullptr && getStylesheetCount() > 0) {
            ESP_UTILS_LOGW("No exact stylesheet match found, using first available stylesheet");
            // Use the stylesheet we just added (360x360) even if display is different
            default_find_data = getStylesheet(gui::StyleSize::RECT(360, 360));
        }
...
bool Settings::calibrateScreenSize(gui::StyleSize &size)
{
    ...
    systems::base::Context *context = getSystemContext();
    if (context == nullptr) {
        ESP_UTILS_LOGW("System context not available yet, skipping calibration");
        return true;
    }

    gui::StyleSize display_size = {};
    ESP_UTILS_CHECK_FALSE_RETURN(context->getDisplaySize(display_size), false, "Get display size failed");
    ESP_UTILS_CHECK_FALSE_RETURN(context->getDisplay().calibrateCoreObjectSize(display_size, size), false, "Invalid screen size");
