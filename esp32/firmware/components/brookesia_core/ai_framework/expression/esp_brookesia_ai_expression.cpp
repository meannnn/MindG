...
#else
    // Anim player disabled - just store maps but don't initialize players
    if (data.flags.enable_emotion && emoji_map) {
        _emoji_map = *emoji_map;
    }
    if (data.flags.enable_icon && system_icon_map) {
        _system_icon_map = *system_icon_map;
    }
#endif
