...
#define ANIM_EVENT_THREAD_STACK_CAPS_EXT    (false)
...

    {
        anim_player_config_t config = {
            .flush_cb = [](anim_player_handle_t handle, int x1, int y1, int x2, int y2, const void *data)
            {
                ...
                // CRITICAL FIX: Always notify flush finished to prevent deadlock
                // Even if validation fails, we must call anim_player_flush_ready() to unblock the animation task
                auto notify_flush_finished = [handle]() {
                    if (handle != nullptr) {
                ...
                if (self == nullptr) {
                    ESP_UTILS_LOGE("Invalid user data in flush callback");
                    notify_flush_finished();  // Prevent deadlock
                    return;
                }
                ...
                if (!(x1 >= 0 && y1 >= 0 && x2 > x1 && y2 > y1 &&
                      x2 <= canvas_config.width && y2 <= canvas_config.height)) {
                    ESP_UTILS_LOGE("Invalid coordinates in flush callback: (%03d,%03d)-(%03d,%03d)", x1, y1, x2, y2);
                    notify_flush_finished();  // Prevent deadlock
                    return;
                }
                ...
                if (data == nullptr) {
                    ESP_UTILS_LOGE("Invalid data pointer in flush callback");
                    notify_flush_finished();  // Prevent deadlock
                    return;
                }
                ...
                // CRITICAL FIX: Wrap signal emission in try-catch to prevent unhandled exceptions
                // boost::signals2 can throw if connected slots throw exceptions
                // Always ensure flush finished is notified, even if exception occurs
                try {
                    flush_ready_signal(x_start, y_start, x_end, y_end, data, self);
                } catch (const std::exception &e) {
                    ESP_UTILS_LOGE("Exception in flush_ready_signal: %s", e.what());
                } catch (...) {
                    ESP_UTILS_LOGE("Unknown exception in flush_ready_signal");
                }
                // Always notify flush finished, even if exception occurred
                // This prevents deadlock if signal throws exception
                notify_flush_finished();
            },
            .update_cb = [](anim_player_handle_t handle, player_event_t event)
            {
                ...
                // CRITICAL FIX: Wrap update callback in try-catch to prevent unhandled exceptions
                // Mutex operations and shared_ptr operations can throw exceptions
                try {
                    auto *self = static_cast<AnimPlayer *>(anim_player_get_user_data(handle));
                    if (self == nullptr) {
                        ESP_UTILS_LOGE("Invalid user data in update callback");
                        return;
                    }
                    ...
                    std::shared_ptr<EventWrapper> event_wrapper = self->_current_event;
                    if (event_wrapper == nullptr) {
                        ESP_UTILS_LOGE("Invalid current event in update callback");
                        return;
                    }

                    // Helper lambda to safely set promise value (prevents double-set)
                    // std::promise::set_value() throws std::future_error if already set
                    auto safe_set_promise = [](std::shared_ptr<EventPromise> promise, int event_index) {
                        if (promise != nullptr) {
                            try {
                                promise->set_value();
                            } catch (const std::future_error &e) {
                                // Promise already satisfied - this can happen if set_value() is called multiple times
                                // Log and continue (this prevents crash but indicates a logic issue)
                                if (e.code() == std::future_errc::promise_already_satisfied) {
                                    ESP_UTILS_LOGW("Promise already satisfied for event %d, skipping", event_index);
                                } else {
                                    ESP_UTILS_LOGE("Promise error for event %d: %s", event_index, e.what());
                                }
                            }
                        }
                    };
                    ...
                    if (event_wrapper->event.operation == Operation::PlayOnceStop) {
                        ...
                        // CRITICAL FIX: Avoid recursive sendEvent() call that causes stack overflow
                        // The recursive call path: update_cb -> sendEvent() -> event_thread -> processEvent() -> 
                        // anim_player_update() -> update_cb -> ... causes infinite recursion and stack overflow
                        // Instead of calling sendEvent() here, just set the promise and let the event thread
                        // naturally handle the next event if there is one. The Stop will happen naturally
                        // when the animation completes.
                        if (self->_event_queue.empty() && !self->_player_flags.is_starting) {
                            // Don't call sendEvent() here - it causes recursion
                            // The animation will stop naturally, and the promise will be set below
                            ESP_UTILS_LOGD("Event queue empty, animation will stop naturally");
                        }
                        
                        // Set promise safely (prevents double-set exception)
                        safe_set_promise(event_wrapper->promise, event_wrapper->event.index);
                        event_wrapper.reset();
                    } else {
                    ...
                    ESP_UTILS_LOGE("Exception in update callback: %s", e.what());
                } catch (...) {
                    ESP_UTILS_LOGE("Unknown exception in update callback");
                }
            },
            .user_data = this,
            .flags = {
            ...
            .task = {
                ...
                .task_stack_caps = static_cast<unsigned>(
                    (data.task.task_stack_in_ext ? MALLOC_CAP_SPIRAM : MALLOC_CAP_INTERNAL) | MALLOC_CAP_8BIT
                ),
            },
        };
            ...
            while (!_event_thread_need_exit)
            {
                ...
                while (!_event_queue.empty() && !_event_thread_need_exit) {
                    ...
                    if (!processEvent(event_wrapper)) {
                        ...
                        // CRITICAL FIX: Set promise on error to prevent deadlock
                        // If processEvent fails, the promise must be set or waitBootAnimationStop() will hang forever
                        if (event_wrapper != nullptr && event_wrapper->promise != nullptr) {
                            try {
                                event_wrapper->promise->set_value();
                            } catch (const std::future_error &e) {
                                // Promise already set, ignore
                                ESP_UTILS_LOGD("Promise already set: %s", e.what());
                            } catch (...) {
                                ESP_UTILS_LOGE("Failed to set promise on error");
                            }
                        }
                    }
                    lock.lock();
...
bool AnimPlayer::processEvent(std::shared_ptr<EventWrapper> event_wrapper)
{
...

    {
        ...
        case Operation::PlayOncePause: {
            ...
            esp_err_t set_src_ret = anim_player_set_src_data(_player_handle, config.data_address, config.data_length);
            if (set_src_ret != ESP_OK) {
                ESP_UTILS_LOGE("Failed to set src data: %s", esp_err_to_name(set_src_ret));
                // CRITICAL FIX: Set promise before resetting event to prevent deadlock
                if (event_wrapper->promise != nullptr) {
                    try {
                        event_wrapper->promise->set_value();
                    } catch (const std::future_error &e) {
