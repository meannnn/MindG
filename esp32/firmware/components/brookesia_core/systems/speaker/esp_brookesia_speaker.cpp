...
bool Speaker::begin(void)
{
    ...
    // Boot animation is optional - if it fails, log warning but continue booting
    // This prevents bootloop when animation partition is missing or corrupted
    if (!_display.startBootAnimation()) {
        ESP_UTILS_LOGW("Start boot animation failed, continuing without animation");
    } else {
        // Note: Boot audio is skipped - audio will be initialized later when Agent starts a chat session
        if (!_display.waitBootAnimationStop()) {
            ESP_UTILS_LOGW("Wait boot animation stop failed, continuing anyway");
        }
    }
