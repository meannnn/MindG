...
namespace esp_brookesia::ai_framework {
    ...
    class Expression {
    public:
        enum { EMOTION_TYPE_NONE = 0, ICON_TYPE_NONE = 0 };
        using EmojiMap = std::map<std::string, std::pair<int, int>>;
        using SystemIconMap = std::map<std::string, int>;
    };
    class Agent {};
}
#endif
