...
audio_prompt_close();
```
> 提示音播放  和 配置播放器是相似的，只是 `提示音播放` 是阻塞式的播放，适合使用短音频的播放。目的是在使用 普通播放 的播放的时候又需要播放
> 提示音，可以有很好响应，并且提示音播放的优先级是最高的
...
- **媒体数据保存（Media Dump）**：
  - UDP 方式：可使用 `script/media_dump_server.py` 脚本接收数据
